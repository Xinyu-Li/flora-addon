...
let LOG_SERVER = webServerUrl + "flora/log_to_db.php"
let date = new Date();
    ...
    if(scaffold_schdule.includes(elapsed_time) ){
		...
		console.log("Scaffold - Sending Request  fo Scaffold: " + block);
            	...
            	if (localStorage.getItem("userclass") == "Generalised"){
                	func =  "GET_GENERALISED_SCAFFOLDS_IMPROVED";
            	}else if (localStorage.getItem("userclass") == "Personalised"){

                     func = "GET_PERSONALISED_SCAFFOLDS_IMPROVED";
                }

            setTimeout(function(){ 
                Scaffold_LogData("Scaffold","Scaffold_Requested", "time at: " + elapsed_time );
                ScaffoldLog("Scaffold","ScaffoldRequested_block_" + block, "time at: " + elapsed_time);
                sPostAjax(webServerUrl + '/functions.php', 'source=' + 'flora' + '&func=' + func +  '&para1=' + localStorage.getItem("uid") + '&para2='  + block + '&para3=' + localStorage.getItem("moodle-lang"), function (_res) {console.log(_res);    localStorage.setItem("Scaff_at_" + block, "promoted") ; Scaffold_LogData("Scaffold","Scaffold_Received", _res); ScaffoldLog("Scaffold","ScaffoldReceived_block_" + block, "time at: " + elapsed_time); promoteScaffold(_res);    });
...
function promoteScaffold(sc){
        ...
        console.log(parsedData);
            if (parsedData.type == "breakpoint"){
                ...
                var showScaffold = (parsedData.option1_enabled == 1) || (parsedData.option2_enabled == 1) || (parsedData.option3_enabled == 1);
                if (showScaffold){
...
function prepare_Scaffold(scf){
  ...
  if (document.getElementById("modal-overlay")){
    if (document.getElementById("_modal")){
        ...
        var  temp_var = localStorage.getItem("tasks-"+ data.userid + "-" + (indx - 1) );

        if ((data.userclass == "Personalised") && (indx >=2) && (temp_var == null) ){
           document.getElementById("imp_ico").style.display = "block";
           document.getElementById("thinkico").style.display = "none";
	       document.getElementById("message2").style.textAlign = "center";
	       document.getElementById("message2").style.marginBottom = "0px";
	}else{
           document.getElementById("imp_ico").style.display = "none";
           document.getElementById("thinkico").style.display = "block";
           document.getElementById("message2").style.textAlign = "left";
           document.getElementById("message2").style.marginBottom = "32px";
         }
    }
  }
}
...
function show_ToDo_List(){
    if((localStorage.getItem("activeScaffold") == null)  || (localStorage.getItem("activeScaffold") == "0")){
        if (data.userlang == 'DE'){
            ...
            alert("Er zijn momenteel geen aanwijzingen.");
	}else{
...
function display_ToDo_List(n){
    ...
    if (document.getElementById("myToDo")){
	    ...
	    document.getElementById("myToDomessage1").innerHTML = m1;
            ...
            document.getElementById("TODOscf_num").innerHTML =  "  | " +  title +  " |";
...
function attach_Scaffold(){
    ...
    text +=  '<div id="timesupImg"> <img id = "srctimesupImg"  src = "https://www.floraproject.org/flora/scaffolds/timesup.jpg" style = "display: block;margin-left: auto; margin-right: auto; height:150px;" /> </div>';
    text +=  '</div>';

    text +=  '<div class="_modal closed" id="_modal">';

    text +=  '<div style="height: 45px;background: #d3d8ea; padding: 15px;text-align: center;">';
    text +=  '<span id="scaffold_title" style="font-weight:bolder; font-size:20px;"> Scaffold Title</span>';
    text +=  '</div>';
    text +=  '<button class="close-button" id="close-button"> X </button>';
    text +=  '<div class="modal-guts">';

    text +=  '<img id="imp_ico" src="https://floralearn.org/flora/scaffolds_imp/important.png" style="width: 60px;height: 60px;float:left;margin-right: 15px;margin-top: -3px;display: none;">';


    text +=  '<p id= "message1" class = "message1"> M1</p>';
    text +=  '<img id= "thinkico" src="https://www.floraproject.org/flora/scaffolds/list.svg" style="width: 40px;height: 40px;float:left; margin-right: 15px; margin-top: -3px; display: block;"/>';
    text +=  '<p id= "message2" class = "message2"> M2</p> ';
    text +=  '<p id = "timeup-pp" style="padding: 0px;display:none;"></p> ';   

    text +=  '<div id = "optionbox" style ="display:none;"><p id="id_options" style="padding-left: 90px;">';
    text +=  '<label><input type="checkbox" name="options" value="option1"><span id="opt_1" style ="display:none; border-right-color: #44749d; border-left-color: #44749d; border-width: 6px;">Option 1</span></label>';
    text +=  '<label><input type="checkbox" name="options" value="option2"><span id="opt_2" style ="display:none; border-right-color: #44749db5; border-left-color: #44749db5;border-width: 5px;">Option 2</span></label>';
    text +=  '<label><input type="checkbox" name="options" value="option3"><span id="opt_3" style ="display:none; border-right-color: #85a1bb7a; border-left-color: #85a1bb7a; border-width: 4px;">Option 3</span></label>';
    text +=  '<label><input type="checkbox" name="options" value="option4"><span id="opt_4" style ="display:none;">Option 4</span></label>';
    text +=  '</p> </div>';
    ...
    text += '<span id ="TODOscf_num" style="width: 165px; font-size:13px;font-weight:bolder;"> Scaffold # : ? </span>';
